...
$query = "SELECT name,time,type FROM doctor where name=?";

if ($stmt = $db->prepare($query)) {
 $stmt->bind_param("s",$name);
 $stmt->execute();
 $stmt->bind_result($name,$time,$type);
 $table1 = <<< TABLE
<table border="1">
  <thead><tr>
   <th>Name</th><th>Time</th><th>Type</th>
  </tr></thead>
  <tbody>
TABLE;
 ...
 while ($stmt->fetch()) {
  ...
  $table1 .= "<tr><td width='100' height='100'>$name</td><td width='100' height='100'>$time</td><td width='100' height='100'>$type</td></tr>";
 }
 $table1 .= "</tbody></table>";
...
<div class="w3-main" style="margin-left:300px">
  ...
  <header id="portfolio">
    ...
    <div class="w3-container">
    <h1><b>Search for Doctors!</b></h1>
    <form method="post" action="searchpage.php">
    <input type="text" name="search" placeholder="Search.." width='500px'>
